...
ResultPacket SyskoSimulation(const QueryPacket &query,
                             ...
                             const size_t num_batches_x,
                             const size_t num_batches_y)
{
    ...
    for (size_t p_id = 0; p_id < num_batches_x; p_id++)
    {
        ...
        size_t x_end_index = p_id == num_batches_x - 1 ? query.x_end_index
                                                       : (p_id + 1) * batch_size_x;

        QueryPacket proc_X = QueryPacket(
            batch_size_x, query.d, x_start_index, x_end_index,
            std::vector<double>(&query.X[idx(x_start_index, 0, query.d)],
                                &query.X[idx(x_end_index, 0, query.d)]));

        ResultPacket proc_res = ResultPacket(0, 0, 0, 0, 0, 0, 0);
        ...
        for (size_t stage = p_id, iters = 0; iters < num_batches_y; iters++)
        {
            ...
            size_t y_end_index = stage == num_batches_y - 1
                                     ? corpus.y_end_index
                                     : (stage + 1) * batch_size_y;

            CorpusPacket proc_Y = CorpusPacket(
                batch_size_y, corpus.d, y_start_index, y_end_index,
                std::vector<double>(&corpus.Y[idx(y_start_index, 0, corpus.d)],
                                    &corpus.Y[idx(y_end_index, 0, corpus.d)]));

            ResultPacket batch_res = knn_blas(proc_X, proc_Y, k);
            proc_res = combineKnnResultsSameX(proc_res, batch_res);
        }
...
ResultPacket runData(const QueryPacket &query, const CorpusPacket &corpus,
                     size_t k)
{
    auto start = std::chrono::high_resolution_clock::now();
    ResultPacket result = knn_blas(query, corpus, k);
...
ResultPacket runDistrData(const QueryPacket &query, const CorpusPacket &corpus,
                          size_t k, size_t num_batches_x,
                          size_t num_batches_y)
{
    auto start = std::chrono::high_resolution_clock::now();
    ResultPacket result =
        SyskoSimulation(query, corpus, k, num_batches_x, num_batches_y);
    auto end = std::chrono::high_resolution_clock::now();
